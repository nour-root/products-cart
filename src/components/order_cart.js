const order_cart = (order, num) => {
  return ` <li
            class="flex items-center justify-between border-b-[1.5px] pb-3 border-gray-200"
          >
            <div class="details">
    ...
    order.price
                ...
                >
                <span id="total-item-${order.id}" class="text-gray-500">$${
    order.price * num
  }</span>
              </div>
            </div>
            <button type="button" id="remove-${
              order.id
            }" onclick="remove_Item('remove-${order.id}')">
              <svg
